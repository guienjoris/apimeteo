...
// API weather
fetch(`://api.openweathermap.org/data/2.5/weather?q=Paris&appid=4bf558d6703a571b73fb51bc44fa5f70&units=metric&lang=fr`)
.then(res => res.json())
.then(data => {
    temperature.textContent=Math.floor(data.main.temp) +"°C";
    humidity.textContent=data.main.humidity +"%";
    lieu.innerHTML=`<a href='://fr.wikipedia.org/wiki/${data.name}' target="_blank"> ${data.name} </a>`;
    climat.textContent= data.weather[0].description;
    icon.textContent= icon.setAttribute('src', "://openweathermap.org/img/w/" + data.weather[0].icon + ".png");
    hour.textContent= Math.floor((date.getHours()-2+data.timezone/60/60)) + ":" +date.getMinutes() +":" + date.getSeconds(); 
});  
// API News
fetch(`://newsapi.org/v2/everything?q=Paris&from=2019-04-23&sortBy=publishedAt&apiKey=5061607fa5594ccbb70f9b710b680cc9`)
.then(res => res.json())
.then(dataNews =>{
...
function requete(){
    ...
    // API weather
    fetch(`://api.openweathermap.org/data/2.5/weather?q=${cityName}&appid=4bf558d6703a571b73fb51bc44fa5f70&units=metric&lang=fr`) 
        .then(res => res.json())
        .then(data => {
            console.log(data);
            temperature.textContent=Math.floor(data.main.temp)+"°C";
    ...
    // API News
        ...
        fetch(`://newsapi.org/v2/everything?q=${cityName}&from=2019-04-23&sortBy=publishedAt&apiKey=5061607fa5594ccbb70f9b710b680cc9`)
        .then(res => res.json())
        .then(dataNews =>{
            console.log(dataNews);
        news_title.innerHTML=  `<a href="${dataNews.articles[3].url}" target="_blank">${dataNews.articles[3].title}</a>` ;
